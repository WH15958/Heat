...
- [ ] 读回或 HMI 显示与写入一致。
- [ ] 地址换算未出现一位偏移。
- [ ] 前端显示的目标设备串口与当前配置和现场身份一致（配置 fallback 为 COM6；指纹解析后的实际端口可能不同），并在写入前要求明确确认。
...
- [ ] start 返回成功时 40151 启动位已读回且 `fault_code=0`，HMI 和物理输出也确认设备确实进入运行。
- [ ] stop 返回成功时启动位已清除且功率/电流归零，HMI 和物理行为也确认设备确实停止。
