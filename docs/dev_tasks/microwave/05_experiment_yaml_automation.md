...
把微波仪接入实验流程，新增 YAML 动作和等待条件。

> 2026-06-20 变更：用户已明确要求取消后端安全边界，自动化实验可像加热器/蠕动泵一样直接 configure/start/stop 微波仪；本文早期“默认禁用 / allow_experiment_control gate”的描述仅保留为历史阶段记录，当前代码和用户文档以后续开放语义为准。
